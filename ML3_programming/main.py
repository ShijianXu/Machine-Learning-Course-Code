...
def Train(train_data, train_targets, weight1, weight2, theta1, theta2, hiddenNum,outputNum,learn_rate=0.1, epochs=20):
    for nn in range(epochs):
        ...
        for k in range(train_data.shape[0]):
            ...
            for i in range(100):
                for j in range(10):
                    weight2[i][j]+=learn_rate*out_g[j]*b[i]
            '''
            ...
            '''
            mat1 = np.outer(x,hidden_e)
            mat1=np.dot(learn_rate,mat1)
            weight1=weight1+mat1
            
            mat2=np.outer(b,out_g)
            mat2=np.dot(learn_rate,mat2)
            weight2=weight2+mat2
            '''
